...
import { Component, Input, Output, EventEmitter, OnInit, OnChanges, ViewChild, Injectable, ComponentFactory} from '@angular/core';
...
export class TableComponent implements OnInit, OnChanges {
    ...
    @ViewChild('pagination') pagination: PagerComponent;
    ...
    public onSortTable(config: any, page: any = { page: this.page, itemsPerPage: this.itemsPerPage }): any {
        ...
        if (this.totalRows > 0 || this.rows.length > 0) {
            let sortCols = config.sorting.columns.filter((x: any) => (x.sort != '') && (x.sort != null))
    ...
    public toggleRowDetail(rowNo: number): any {
        let component = this.ngTableComponent.toggleRowDetail(rowNo);
