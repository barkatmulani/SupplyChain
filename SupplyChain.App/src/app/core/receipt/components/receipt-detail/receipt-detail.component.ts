...
export class ReceiptComponent extends BaseDetailComponent implements OnInit {
  ...
  loadPurchaseOrderInfo(rec: any) {
    if(rec) {
      ...
      if(!this.receiptId) {
        this.receiptItems = rec.purchaseOrderItem.map((item: any) => ({
          ...item,
        ...
        this.receiptItems = this.receiptItems.map(receiptItem => {
          const item = rec.purchaseOrderItem.find((poItem: any) => poItem.itemId == receiptItem.itemId);
