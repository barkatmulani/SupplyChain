...
export class NgTableComponent implements OnInit, OnChanges {
    ...
    containers = new QueryList<ViewContainerRef>();
    ...
    public cellClick(event: any, row: any, column: any): void {
        if (this.allowMultiSelect) {
            ...
            this.dataRows.map(r => r.className = '');
        ...
        this.cellClicked.emit({ row, column, selectedIds: this.selectedIds, selectedId: this.selectedId });
    }
    ...
    onCheckboxClicked(id: any): void {
        ...
        if (this.checkedId !== '') {
            if (this.checkedId !== (this.dataRows[parseInt(this.idColumn)] || this.dataRows[0].id))
    ...
    onChangeDropdown(id: string, row: any, columnName: string, colNo: number) {
    ...
    validateCells(colNo: number, rowNo: number|undefined = undefined): any {
        let len: number;
        let message: string;
        let matrixCol: any[];
    ...
    private addMessage(type: string, colNo: number, rowNo: number = 0, sourceRowNo: number = 0, message: string = '') {
    ...
    private removeMessage(type: string, colNo: number, rowNo: number = 0, sourceRowNo: number = 0) {
        if (!sourceRowNo) sourceRowNo = rowNo;
